...
| ext-shared-rate-status | div | **New 2026-07-30.** Row 2 of the sidebar, below `.ext-sidebar-row1`. Visible only when "Shared refresh limit" is ON **and** the rate limiter is not paused (while paused, row 1's `ext-rate-limit-banner` already communicates it — this line hides rather than duplicating it). Uses the banner's own `isRateLimitPaused()` as of 2026-07-30, rather than its own `backoffUntil` comparison, so the two can never disagree. Text: `"1 active tab → refreshing every X.Xs"` when N=1, else `"Active tabs: N → each tab refreshes every X.Xs"`, X = interval × N. N comes from `ACTIVE_TAB_COUNT_KEY` (`extActiveTabCount`) — background.js's own active-tab registry, live-synced via `chrome.storage.onChanged`, the same source (not a second counter) the permit system's tab-heartbeat mechanism maintains. Written by `renderSharedRateStatus()`. |
| ext-rate-limit-banner | span[role=status] | Paused banner, hidden by default, shown in row 1 in place of `ext-slider-speed` + `ext-slider-value` whenever `background.js` reports a rate-limited state. Fixed amber `#d4a72c` (semantic caution signal, deliberately not a `--ext-*` token, so it survives theme changes — same convention as the Fast Book button's fixed blue). **A flex container as of 2026-07-30**, holding `ext-rate-limit-text` + `ext-rate-limit-info`, so the "i" sits immediately after the sentence rather than being pushed right by `flex:1`. **`ext-playpause` stays visible alongside it** — see that row. Visibility is decided by `isRateLimitPaused()`, driven purely by `chrome.storage.onChanged`; there is no longer any timer behind this element. |
| ext-rate-limit-text | span | The banner sentence, set via `textContent`, **static as of 2026-07-30**: *"Paused — Amazon has temporarily limited your IP due to frequent refreshes. Access returns on its own; the extension will resume automatically."* It previously ended in a live `"Retrying in Xs"` countdown of `background.js`'s own backoff timer — removed because that number described *our* next retry, not Amazon's unblock time, reset on every page reload, and meant nothing on reaching zero. Carries the `text-overflow:ellipsis` (moved off the banner) so truncation eats the sentence, never the icon. |
| ext-rate-limit-info | span | Small circled "i" at the end of the banner, `tabindex="0"`. Hover (mouseenter/leave), focus/blur, and click/tap all toggle `ext-rate-limit-tooltip` — identical convention to `ext-memory-info`, and it shares that element's geometry/focus-ring CSS rules. Overrides fill and border to `currentColor` so it reads as part of the amber banner (which is why it needs no `html.ext-night` override). |
| ext-rate-limit-tooltip | div | Child of `ext-rate-limit-info`, shown via the shared `.ext-tooltip-visible` class, `textContent` only, `role="tooltip"`. 340px wide (vs the memory tooltip's 220px — the copy is ~4× longer). Explains that Amazon throttles per-IP refreshes of the load board, that it is neither an account problem nor an extension fault, that it clears by itself, and points at the **Shared refresh limit** setting as the way to avoid it. Force-closed by `updateRateLimitDisplay()` when the paused state ends, so it is never left orphaned over a hidden banner. |
