...
| ext-origin-cities-all | div (button) | **"All" button** (2026-08-13, replaced `ext-origin-cities-title`). Clears the city filter — shows every load from every selected city. `role="button"`, `tabindex="0"`, Enter/Space. Styled as a pill like the cities; `flex-shrink:0` keeps it whole when they wrap. Carries `data-active="true"` when no city filter is applied, which is the default state. |
