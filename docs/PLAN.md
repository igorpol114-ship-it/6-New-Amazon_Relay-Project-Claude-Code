...
4. **Rebuild the harness suites on the real DOM structure.** next — **scope grew again**: `citydrop-harness` now crashes too (five of its eight sections drive the retired clone path, and its stubs are plain objects rather than `Response` instances), on top of the four earlier suites that stub the summary panel as an ancestor, provide no `getElementById`, and use non-UUID fixture ids. Green and current: `citysibling-harness` (56) and `citypiggy-harness` (53). *Verify: a full test run comes back green, no red noise.*
5. **Audit `loadParser.js:124`** — same "first `div.load-list`" assumption, feeds highlight and sound. next. *Verify: highlight and alert fire only on main-list loads, never on a Similar-matches card.*
